...
// import NotFound from './ErrorNotFound404';
...
class Admin_ManageProduct extends Component {
  ...
  componentDidMount() {
    axios
      .get(`${API_URL}/products?_expand=category&_page=${this.state.page}&_limit=6`)
      .then((res) => {
        axios
    ...
    if (this.state.products.length === 0) {
      return (
        <div
          ...
          style={{ height: "63vh", width: "243%", backgroundColor: "lightgray" }}
        >
    ...
    return this.state.products.map((val, index) => {
      ...
      return (
        <tr key={val.id} style={{ fontWeight: '500', fontSize: '13px' }}>
          <td width="50px" style={{ verticalAlign: "middle" }}>{Date.now()}</td>
          <td width="50px" style={{ verticalAlign: "middle" }}>{val.nama}</td>
          <td width="50px" style={{ verticalAlign: "middle" }}>{val.serial}</td>
          <td width="50px" style={{ verticalAlign: "middle" }}>{val.stock}</td>
          <td width="100px" style={{ verticalAlign: "middle", textAlign: 'center' }}>{currencyFormatter(val.price)}</td>
          <td width="250px" style={{ verticalAlign: "middle" }}>{val.status}</td>
          <td width="120px" style={{ verticalAlign: "middle", textAlign: 'center' }}>{val.category.nama}</td>
          <td width="100px" style={{ verticalAlign: "middle", textAlign: 'center' }}>
            <span
  ...
  renderPaging = () => {
    ...
    for (let i = 0; i < berapaPaging; i++) {
      if (i + 1 === page) {
        ...
        paging.push(
          <PaginationItem onClick={() => this.setState({ page: (i + 1) })} className="mb-5 mt-5 ml-1 mr-1">
            <button
              className="btn"
              style={{
                borderRadius: '50%',
  ...
  onAddDataClick = () => {
    ...
    if (nama && serial && stock && price && status && categoryId) {
      axios
  ...
  onSaveEditClick = () => {
    ...
    if (nama && stock && price && serial && categoryId) {
      axios
      ...
      <div>
        <Modal size="md" isOpen={this.state.modalAdd} toggle={this.toggle}>
          ...
          <ModalFooter className="pr-2">
            <Button className="px-4 py-2 tombol-add2" onClick={this.onAddDataClick}>
              Add
            </Button>
          </ModalFooter>
        </Modal>
        ...
        <Navbar className="header-bg px-5 py-5" expand="md">
          <NavbarBrand href="/" style={{ color: 'rgb(50, 50, 50)' }}> Home </NavbarBrand>
        ...
        <div className="mx-5 my-1">
          ...
          <Table className="mt-3" bordered hover responsive>
            <thead style={{ textAlign: 'center', backgroundColor: 'rgb(250, 250, 250)' }}>
